...
// TestPostgresSequenceOrder: two engines share a database and must still run a
// sequence strictly in insertion order.
func TestPostgresSequenceOrder(t *testing.T) {
	dsn := testDSN(t)
	resetDB(t, dsn)
	a := openQ(t, dsn)
	b := openQ(t, dsn)
	...
	var mu sync.Mutex
	var order []int
	task := quacker.NewTask("pg.seq", func(ctx context.Context, in int) (int, error) {
		mu.Lock()
		order = append(order, in)
		mu.Unlock()
		return in, nil
	}, quacker.WithSequence(func(in int) string { return "s" }))
	quacker.Register(b, task) // so either engine can execute
	...
	if len(order) != len(inputs) {
		t.Fatalf("ran %d, want %d", len(order), len(inputs))
	}
	for i, v := range order {
		if v != i {
...
// TestPostgresMultipleKeys: the extra-keys gate holds across engines.
func TestPostgresMultipleKeys(t *testing.T) {
	dsn := testDSN(t)
	resetDB(t, dsn)
	a := openQ(t, dsn)
	b := openQ(t, dsn)
	...
	started := make(chan string, 4)
	release := make(chan struct{})
	task := quacker.NewTask("pg.mk", func(ctx context.Context, in mkInput) (string, error) {
		started <- in.Acct
		<-release
		return in.Acct, nil
	},
	...
	if s, err := b.Execution(ctx, h2.RunID()); err != nil || s.Status != quacker.StatusQueued {
		t.Fatalf("second run = %+v err=%v, want QUEUED (account key saturated)", s, err)
	}
	close(release)
